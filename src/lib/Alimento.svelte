...
<a class="block" href="/alimenti/{row.id}">
	<div class="card card-hover mx-auto p-8">
		...
		{/if}
		{#if row.scadenza && moment().isAfter(row.scadenza, 'day')}
			<div class="text-xl mb-2 chip variant-filled-surface">⚠️ Scaduto ⚠️</div>
		{/if}
		{#if row.scadenza && moment().isSame(row.scadenza, 'day')}
			<div class="text-xl mb-2 chip variant-filled-surface">⚠️ Scade oggi ⚠️</div>
		{/if}
